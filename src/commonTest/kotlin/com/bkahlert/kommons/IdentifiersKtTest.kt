...
class IdentifiersTest {
    ...
    @Test fun should_replace_leading_digit_with_lower_case_if_majority_amount_of_letters_are_lower_case() = test {
        "0_Abc123".toIdentifier() shouldBe "o_Abc123"
        "1_Abc123".toIdentifier() shouldBe "i_Abc123"
        "2_Abc123".toIdentifier() shouldBe "z_Abc123"
        "3_Abc123".toIdentifier() shouldBe "b_Abc123"
    ...
    @Test fun should_keep_alphanumeric_period_underscore_and_dash() = test {
        "aB3._-xx".toIdentifier() shouldBe "aB3._-xx"
    }

    @Test fun should_replace_whitespace_with_dash() = test {
        "abc- \u2000".toIdentifier(6) shouldBe "abc---"
    }

    @Test fun should_replace_with_underscore_by_default() = test {
        "a𝕓☰👋⻦𐦀︎".toIdentifier() shouldBe "a_______"
    }

    @Test fun should_produce_same_basename_for_same_input() = test {
        "abc".toIdentifier() shouldBe "abc".toIdentifier()
    }

    @Test fun should_produce_same_basename_for_high_length() = test {
        "abc".toIdentifier(1000) shouldBe "abc".toIdentifier(1000)
    }

    @Test fun should_return_constant_for_null() = test {
